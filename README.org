...
** Viewing Phylip/BPP Format Alignments

=sequed-phy-mode= is automatically activated for files with a =.phy= extension. It provides syntax highlighting for phylip/bpp format sequence data (header lines, sequence labels and DNA bases).

*** Opening the Alignment Viewer
From a buffer in =sequed-phy-mode= use the key combination =C-c C-a= or =M-x sequed-phy-mkaln= to open the alignment viewer. This creates a read-only =*Phy Alignment Viewer*= buffer in =sequed-phy-aln-mode= showing the first locus with colored DNA bases and yellow sequence labels. The mode line displays the current sequence name (SeqID), base position (BasePos) and locus number (Locus).

*** Navigating Between Loci
Multi-locus phylip files (as used by BPP) are supported. Each locus has its own header line specifying the number of sequences and sites. The following key combinations navigate between loci:

| Key       | Command                        | Description               |
|-----------+--------------------------------+---------------------------|
| =C-c C-n= | =sequed-phy-aln-next-locus=    | Display the next locus    |
| =C-c C-p= | =sequed-phy-aln-prev-locus=    | Display the previous locus |
| =C-c C-g= | =sequed-phy-aln-goto-locus=    | Jump to a locus by number |

*** Extracting Loci
To extract a range of loci into a new buffer in valid phylip format use =C-c C-x= or =M-x sequed-phy-aln-extract-loci=. You will be prompted for the first and last locus numbers (1-based, inclusive). The new =*Extracted Loci*= buffer is opened in =sequed-phy-mode=.

*** Inherited Commands
=sequed-phy-aln-mode= inherits several commands from =sequed-aln-mode=:

| Key       | Command                        | Description                      |
|-----------+--------------------------------+----------------------------------|
| =C-c C-b= | =sequed-aln-gotobase=          | Move cursor to a specific base   |
| =C-c C-f= | =sequed-aln-seqfeatures=       | Show sequence count and length   |
| =C-c C-k= | =sequed-aln-kill-alignment=    | Close the alignment viewer       |
